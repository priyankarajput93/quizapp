...
function RadioButton({optionList, questionArray, disable}) {
  ...
  useEffect(() => {
    if (disable !== undefined) setOption(questionArray.selectedAnswer);
  }, []);

  return (
    <View style={styles.optionConatiner}>
      {optionList.map(optionValue => (
        <View key={optionValue} style={styles.quizConatiner}>
          <TouchableOpacity
            style={styles.outer}
            disabled={disable}
            onPress={() => {
              ...
              if (questionArray === undefined) {
                ...
                questionArray.isCorrectAnswer =
                  optionValue == questionArray.correct_answer ? true : false;
                questionArray.selectedAnswer = optionValue;
              }
            }}>
            {selectedOption === optionValue && <View style={styles.inner} />}
          </TouchableOpacity>
          <Text style={styles.option}>{optionValue}</Text>
        </View>
      ))}
    </View>
  );
